...
def find_constant_columns(df: pd.DataFrame) -> List[str]:
    """
    Найти колонки, в которых все не нулыевые значения одинаковы.
    Возвращает список имен таких колонок.
    """
    constant_cols = []
    for col in df.columns:
        non_null_vals = df[col].dropna()
        if len(non_null_vals) > 0 and non_null_vals.nunique() == 1:
            constant_cols.append(col)
    return constant_cols

def find_suspicious_id_duplicates(df: pd.DataFrame) -> bool:
    """
    Проверить, есть ли дубликаты в колонках идентификаторов.
    Возвращает True, если найдены дубликаты в ID-колонках.
    """
    # Паттерны для поиска ID-колонок
    id_patterns = ['id', 'Id', 'ID', '_id', 'Id_']
    
    for col in df.columns:
        col_lower = col.lower()
        if any(pattern in col_lower for pattern in id_patterns):
            non_null_vals = df[col].dropna()
            if len(non_null_vals) > 0 and non_null_vals.duplicated().any():
                return True
    return False
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    flags["has_suspicious_id_duplicates"] = find_suspicious_id_duplicates(df)
    ...
    # Уменьшаем оценку за одинаковые колонки и дубликаты ID
