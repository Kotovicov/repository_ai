...
### 4. `POST /quality-flags-from-csv` – Полный набор флагов качества по CSV-файлу
...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- возвращает эвристики датасета в формате:
```
{
  "flags": {
    "too_few_rows": true,
...
- `too_many_columns` - много колонок;
- `too_many_missing` - много пропусков;
- `has_constant_columns` - есть столбцы с одинаковыми значениями;
- `has_suspicious_id_duplicates` - есть свопядения id.
